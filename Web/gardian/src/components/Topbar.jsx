...
export default function Topbar() {
  ...
  const toggleNotif = () => {
    ...
    if (!isNotifOpen) setHasUnread(false); // remove red dot when opened
  };
        ...
        <div className="relative" ref={notifRef}>
          <button
            ...
            className={`relative p-2 rounded-full transition ${
              isNotifOpen ? "bg-gray-200 text-gray-800" : "hover:bg-gray-100 hover:text-gray-800"
            }`}
          >
            <FaBell
              className={`text-xl transition ${
                isNotifOpen ? "text-gray-800" : "text-gray-200"
              }`}
            />
            {hasUnread && (
              ...
              <ul className="max-h-60 overflow-y-auto">
                {notifications.length > 0 ? (
                  notifications.map((n) => (
                    <li
                      key={n.id}
                      className="px-4 py-3 hover:bg-gray-50 border-b text-sm text-gray-700"
                    >
                      <p>{n.message}</p>
